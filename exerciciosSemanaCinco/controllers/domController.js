...
import {createInteractiveScore} from "../script/score.js";
...
async listMovies (movies) {
    ...
    container.innerHTML = `<button on clicked() id="addmovie-btn" class="card">< i class="ph-plus"></i></button>`;
    const cards = await Promise.all(
        movies.map(async(movie)=> await this.#creatCard(movie))
    );
